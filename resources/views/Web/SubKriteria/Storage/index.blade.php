                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                <a href="#" class="btn btn-warning btn-edit" data-toggle="modal" data-id="{{ $item->idx_storage }}" data-target="#modaledit">Edit</a>
                                                <a href="{{ url('/store/ ' . $item->idx_storage . '/delete ') }}" class="btn btn-danger">Delete</a>
                                            </div>
                                        </td>
                                        </tr>
                                        @endforeach
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Tambah Data Storage</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form method="POST" action="{{ route('poststorage') }}">
                        @csrf
...
$('.btn-update').on('click',function () {
    // console.log($(this).data('id'))
    let idx_storage = $('#editForm').find('#idx_storage').val()
    let formData = $('#editForm').serialize()
    $.ajax({
        url: `/store/${idx_storage}/update`,
        method:"POST",
        data : formData,
        success: function (data) {
        console.log(data)
